...
class Expensesnaway
 ...
 $this->db->query("SET sql_mode=(SELECT REPLACE(@@sql_mode, 'ONLY_FULL_GROUP_BY', ''));");
        $this->load->model('expensesnaway_model', 'expenses');
    ...
    public function index() {
        ...
        $daily = $this->db->query("Select SUM(daily) as daily, MONTHNAME(date) as date from expensesnaway where YEAR(date) = '" . date('Y') . "' group by MONTH(date)");
        ...
        $this->template->addView("expensesnaway/all", 'body');
    ...
    public function create() {
        ...
        if ($this->expenses->create()) {
            redirect('expensesnaway');
        }

        $this->template->addView("expensesnaway/create", 'body');
        $this->template->display();
    }
    ...
    public function view($id) {
        ...
        $this->template->addView("expensesnaway/view", 'body');
        $this->template->display();
    }
    ...
    public function edit($id) {
        ...
        $this->template->assign('r', $result[0]);
        $this->template->addView("expensesnaway/edit", 'body');
        $this->template->display();
    }
