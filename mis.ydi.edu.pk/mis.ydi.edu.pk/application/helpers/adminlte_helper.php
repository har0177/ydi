...
class AdminLTE {
   ...
   public static function rank($id, $course) {
    ...
    $que = $CI->db->query("
        SELECT FIND_IN_SET(percentage, (
            SELECT GROUP_CONCAT(DISTINCT percentage ORDER BY percentage DESC)
            FROM trainer_data
            WHERE course = $course
              AND YEAR(date) = $date_year
              AND WEEKOFYEAR(date) = WEEKOFYEAR(CURDATE())
        )) AS rank_no
        FROM trainer_data
        WHERE regno = $id
          AND YEAR(date) = $date_year
    ...
    if ($que->num_rows() > 0) {
        ...
        $que = $CI->db->query("
            SELECT FIND_IN_SET(percentage, (
                ...
                WHERE course = $course
                  AND YEAR(date) = $date_year
                  AND WEEKOFYEAR(date) = WEEKOFYEAR(CURDATE()) - 1
            )) AS rank_no
            FROM trainer_data
            WHERE regno = $id
              AND YEAR(date) = $date_year
     ...
     public static function rank_ydi($id) {
    $CI = & get_instance();
    $date_year = date('Y');
...
public static function rank_date($id, $course, $date) {
    ...
    public static function rank_date_ydi($id, $date) {
    $CI = & get_instance();
    $date_year = date('Y');
    ...
    public static function exp_name($id) {
      ...
      public static function user_course($id) {
        $CI = & get_instance();
        $CI->db->select('status');
        $array = array('course_id' => $id);
        $CI->db->where($array);
    ...
    public static function report_info($cur = "") {
        $CI = & get_instance();
        $q = $CI->db->query("SELECT * from trainer_data where regno = '$cur' and WEEKOFYEAR(date)=WEEKOFYEAR(CURDATE())");
        $r = $q->result();
        return $r;
    }
	
	  public static function monthly_report_info($cur = "") {
        $CI = & get_instance();
		$month = date('m');
        $q = $CI->db->query("SELECT * from report_naway where regno = '$cur' and MONTH(date) = $month");
        $r = $q->result();
        return $r;
    }
    ...
    public static function salary_dues($id) {
        $CI = & get_instance();
     $query =   $CI->db->query("Select * from salary where (status = 0 OR status = 3) and emp_id = $id order by id DESC LIMIT 1");
        if ($query->num_rows() == 1) {
            $result = $query->result_array();
            return $result[0]['dues'];
        } else {
    ...
    public static function employees($cur = "") {
        $CI = & get_instance();
    ...
    public static function Inter_student_info($id) {
        $CI = & get_instance();
        $CI->db->where(array('regno' => $id, 'status' => 1));
        $q = $CI->db->get('interview');
        return $q->row();
    }
...
// Send the POST request with cURL
     ...
     public static function sms_bulk($nos, $msg) {
